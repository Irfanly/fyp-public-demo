...
import { PlusCircle, Edit2, Clipboard, ArrowRight, CheckCircle2, Trash, User, ChevronDown, ChevronUp, Eye, X } from "lucide-react";
...
const TaskDashboard = ({ eventID } : { eventID : string}) => {
  ...
  const handleDeleteTask = async (task: tasks) => {
    if (task.taskID) {
      //delete task from Firestore
      try {
        await firestore.deleteTask(task.taskID);
        // Update tasks state
        if (tasks) {
